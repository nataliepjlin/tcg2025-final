...
template<class SrcIter, class DestIter>
SrcIter
uneven_copy_impl(SrcIter src_first, DestIter dest_first, DestIter dest_last, std::false_type)
{
    typedef typename std::iterator_traits<SrcIter>::value_type src_t;
    typedef typename std::iterator_traits<DestIter>::value_type dest_t;
...
inline SrcIter uneven_copy(SrcIter src_first, DestIter dest_first, DestIter dest_last)
{
    ...
    return uneven_copy_impl(
        src_first, dest_first, dest_last, std::integral_constant<bool, DEST_IS_SMALLER>{}
    );
}
...
void generate_to_impl(SeedSeq &&generator, DestIter dest, std::false_type)
{
    ...
    constexpr size_t FROM_ELEMS   = GEN_IS_SMALLER ? size * ((DEST_SIZE + GEN_SIZE - 1) / GEN_SIZE)
...
inline void generate_to(SeedSeq &&generator, DestIter dest)
{
    ...
    generate_to_impl<size>(
        std::forward<SeedSeq>(generator), dest, std::integral_constant<bool, IS_32BIT>{}
    );
}
