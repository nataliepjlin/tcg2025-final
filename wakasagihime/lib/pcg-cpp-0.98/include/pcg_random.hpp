...
class unique_stream {
    ...
    constexpr itype increment() const { return itype(reinterpret_cast<unsigned long>(this) | 1); }
  ...
  , protected multiplier_mixin {
    ...
    engine(
        ...
        typename std::enable_if<
            !stream_mixin::can_specify_stream && !std::is_convertible<SeedSeq, itype>::value &&
    ...
    template<
        typename xtype1,
        typename itype1,
        typename output_mixin1,
        bool output_previous1,
        typename stream_mixin_lhs,
        typename multiplier_mixin_lhs,
        typename stream_mixin_rhs,
...
using oneseq_base =
    engine<xtype, itype, output_mixin<xtype, itype>, output_previous, oneseq_stream<itype>>;
...
template<
    ...
    bool output_previous = (sizeof(itype) <= 8)>
using unique_base =
    engine<xtype, itype, output_mixin<xtype, itype>, output_previous, unique_stream<itype>>;
...
template<
    ...
    bool output_previous = (sizeof(itype) <= 8)>
using setseq_base =
    engine<xtype, itype, output_mixin<xtype, itype>, output_previous, specific_stream<itype>>;
...
struct inside_out : private baseclass {
    ...
    static_assert(
        sizeof(result_type) == sizeof(state_type),
    ...
    static bool
    external_advance(result_type &randval, size_t i, result_type delta, bool forwards = true)
    {
        state_type state        = baseclass::unoutput(randval);
...
)
{
    ...
    static_assert(
        basebits <= extbits || advance_pow2 > 0, "Current implementation might overflow its carry"
    );
