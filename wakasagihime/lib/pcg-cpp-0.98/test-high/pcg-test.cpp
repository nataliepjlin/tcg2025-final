...
int main(int argc, char **argv)
{
    ...
    constexpr int how_many_nums = bits <= 8 ? 14 : bits <= 16 ? 10 : 6;
    constexpr int wrap_nums_at  = bits > 64 ? 2 : bits > 32 ? 3 : how_many_nums;
    ...
    for (int round = 1; round <= rounds; ++round) {
        ...
        for (int i = 0; i < how_many_nums; ++i) {
            if (i > 0 && i % wrap_nums_at == 0)
                cout << "\n\t";
            cout << " 0x" << hex << setfill('0') << setw(sizeof(RNG::result_type) * 2) << rng();
        }
        cout << endl;
        ...
        cout << "\n   -->   rolling dice used " << (rng - rng_copy) << " random numbers" << endl;
...
int main()
{
    ...
    cout << "Sorry, " STRINGIFY(RNG) " not supported with emulated 128-bit math" << endl;
}
